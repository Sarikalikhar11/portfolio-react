...
const NavBar = () => {
  ...
  return (
    ...
    <div className="flex justify-between items-center w-full h-20 bg-black px-4 text-white fixed">
       <div>
            <h1 className="text-6xl font-signature ml-2">Sarika</h1>
       </div>
       <ul className="hidden md:flex">
              {links.map(({id, link}) => (
            <li key={id} className="px-4 text-2xl cursor-pointer font-medium text-gray-400 hover:scale-105 duration-200">{link}</li>
            ))}
       </ul>
       ...
       {links.map(({id, link}) => (
            <li key={id} className="px-4 cursor-pointer py-6 text-4xl">{link}</li>
            ))}               
       </ul>
            )}
    </div>
